  ...
  return <main className="min-h-screen text-slate-100 selection:bg-violet-400/30">
    ...
    <div className="relative mx-auto max-w-6xl px-5 pb-16 pt-7 sm:px-8">
      ...
      {!result ? <section className="mx-auto max-w-3xl pt-8 text-center">
        ...
        <aside className="order-first lg:order-none"><div className="sticky top-7 rounded-2xl border border-white/8 bg-slate-900/50 p-5"><p className="text-xs font-semibold uppercase tracking-[.16em] text-slate-500">Video context</p><div className="mt-5 space-y-4 text-sm text-slate-300"><Meta icon={<Clock3 size={16}/>} label="Duration" value={time(result.video.duration_seconds)}/><Meta icon={<CheckCircle2 size={16}/>} label="Captions" value={result.video.transcript_language.toUpperCase()}/><Meta icon={<Bot size={16}/>} label="Indexed" value={`${result.indexed_chunks} passages`}/></div><div className="mt-6 border-t border-white/8 pt-5"><p className="text-xs leading-5 text-slate-500">Every response is constrained to retrieved transcript passages.</p></div></div></aside>
      </section>}
    </div>
  </main>
}
